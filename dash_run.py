...
# hs_data = pd.read_csv(os.path.join(base_path, "1028_HSCD.csv"), dtype={'MTICD':'string','HSCD':'string'})

data = pd.read_csv("MTI_KO.csv", dtype={'MTICD':'string'})
hs_data = pd.read_csv("1028_HSCD.csv", dtype={'MTICD':'string','HSCD':'string'})
...
def hs_output_div(df_dict, active_cell, cur_page, page_size):
    ...
    if active_cell is None:
        ...
        if col == 'MTI_NAME':
            result = hs_data[hs_data['MTI_NAME']==active_data]
        else:
            result = hs_data[hs_data['MTICD']==active_data]
